...
if (isset($_POST['save'])) {
    ...
    $date = htmlspecialchars(mysqli_real_escape_string($connection, $_POST['date_of_birth']));
    ...
    if (empty($fullname) or empty($matric) or empty($department) or empty($course) or empty($mode) or empty($graduation) or empty($entry) or empty($select) or empty($marry) or empty($email) or empty($phone) or empty($date) or empty($address)) {
        ...
        $sql = "UPDATE userdata SET name = '$fullname',jamb_reg = '$matric',department = '$department', address = '$address', course = '$course',mode_of_entry = '$mode',graduation_year = '$graduation',year_of_entry = '$entry', current_level='$level',gender = '$select',marital_status = '$marry',email_address  = '$email',phone_no = '$phone',signature = '$sign',photo = '$pass',date_of_birth = '$dates', qr_code = '$file' where jamb_reg='$matric' or id='$id'";
        $query = mysqli_query($connection, $sql);
        if ($query) {
            $_SESSION['quer'] = $query;
            echo '<script>
            window.setTimeout(function() {
    window.location.href = "prinout2.php";
}, 5000);
            </script>';
...
">
            ...
            <div id="layoutSidenav_content">
                ...
                <h3 class="mt-4"> Welcome Back to FNAS NSUK,  <?php echo $dash['name'] . " | " . $dash['jamb_reg']; ?></h3>
                        <p>(Please update Jamb Reg. No with your Matric No. when you pay school fees in other to get your FNAS ID CARD)</p>
                        <p>After you uploaded your Passport and Signature Please Re-login using your Email and phone Number for your information to be updated before printing</p>
                             <?php
$quote = array(
...
?>
  ...
  <div class="col-md-4 my-2">
      <form class="shadow-sm " action="registration2.php" method="post" enctype="multipart/form-data">
      <label for="name">Full Name</label>
    <input type="text" class="form-control" required  name ="fullname" readonly placeholder="First name" value="<?php echo $dash['name']; ?>" aria-label="First name">
  </div>
    <div class="col-md-4 my-2">
         <label for="name">Jamb Reg No/Matric No</label>
    <input type="text" class="form-control" required  name ="matric" value="<?php echo $dash['jamb_reg']; ?>" placeholder="Last name" >
  </div>
  <div class="col-md-4 my-2">
         <label for="name">Department</label>
    <input type="text" class="form-control" readonly required required name ="department" value="<?php echo $dash['department']; ?>">
  </div>
  <div class="col-md-4 my-2">
         <label for="name">Course of Study</label>
    <input type="text" class="form-control" readonly required  name ="course" value="<?php echo $dash['course']; ?>" placeholder="Couse of Study" >
  </div>
  <div class="col-md-4 my-2">
         <label for="name">Mode of Entry</label>
    <input type="text" class="form-control" required readonly  name ="mode" value="<?php echo $dash['mode_of_entry']; ?>" placeholder="Mode of Entry" aria-label="Mode of Entry">
  </div>
  <div class="col-md-4 my-2">
         <label for="name">Year of Graduation</label>
    <input type="text" class="form-control" required   name ="graduation" value="<?php echo $dash['graduation_year']; ?>" placeholder="Year of Graduation" >
  </div>
  <div class="col-md-4 my-2">
         <label for="name">Year of Entry</label>
    <input type="text" class="form-control" required  name ="entry" value="<?php echo $dash['year_of_entry']; ?>" placeholder="Year of Entry" >
  </div>
  <div class="col-md-4 my-2">
  ...
  <input type="text" class="form-control" required  name ="email" value="<?php echo $dash['email_address']; ?>" placeholder="fnasnsuk@example.com">
</div>
  </div>
  <div class="col-md-4 my-2">
  ...
  <input type="text" class="form-control" required name="phone" value="<?php echo $dash['phone_no']; ?>" placeholder="Phone Number">
</div>
  </div>
  <div class="col-md-4 my-2">
       <label for="sign">Upload Your Signature(Max: 500KB)</label>
  <input class="form-control"  oninput="this.setCustomValidity('')" oninvalid="this.setCustomValidity('Please Upload your Signature')" required  name ="sign" type="file" id="formFile">
</div>
  <div class="col-md-4 my-2">
  ...
  <input type="date" class="form-control" required value = <?php echo $dash['date_of_birth']; ?> name="date_of_birth" id="">
</div>
  <div class="col-md-4 my-2">
       <label for="address">Contact Address</label>
  <input type="text" class="form-control" required name="address" value="<?php echo $dash['address']; ?>">
</div>
<div class="col-md-4 my-2">
...
<div class="col-md-4 my-2">
    <input type="hidden" class="form-control" required  name ="id" value="<?php echo $dash['id']; ?>" placeholder="ID" >
  </div>
<button class="btn btn-primary w-50 my-5 m-auto text-center" name="save" type="submit">Save</button>
                ...
                <?php
if (isset($_SESSION['quer'])) {
    echo '<script>
    swal({
  title: "Success",
  text: "Your Information has been Saved Successfully. Your Printout is Loading... ",
  icon: "success",
  timer: 4800,
  button: "OK",
});
